...
void MemoryManager::initialize()
{
    ...
    for(int i = 0; i < 1024; i++){
        pageTableLocks[i].initialize();
    }

    //initialize PageQueue
    max_page_size = kernelPages;
    this->pageQueue = new PageQueue[max_page_size];
}
...
{
    // 互斥访问页表
    int pdIndex = (virtualAddress >> 22) & 0x3FF;
    pageTableLocks[pdIndex].lock();
    ...
    *pte = physicalPageAddress | 0x7;
    ...
    // 加入FIFO页面追踪队列
    enqueuePage(virtualAddress, physicalPageAddress);
...
void MemoryManager::enqueuePage(int vaddr, int paddr) {
    if (pageCount >= max_page_size) return; // 防溢出
    pageQueue[queueTail] = {vaddr, paddr};
    queueTail = (queueTail + 1) % max_page_size;
    ++pageCount;
}

PageQueue MemoryManager::dequeuePage() {
    PageQueue page = pageQueue[queueHead];
    queueHead = (queueHead + 1) % max_page_size;
    --pageCount;
    return page;
}
...
void MemoryManager::replacePage()
{
    ...
    PageQueue oldPage = dequeuePage();

    // 清除页表项
    int *pte = (int *)toPTE(oldPage.vaddr);
    *pte = 0;

    releaseVirtualPages(AddressPoolType::KERNEL, oldPage.vaddr, 1);

    // 释放物理页
    releasePhysicalPages(AddressPoolType::KERNEL, oldPage.paddr, 1);
}
