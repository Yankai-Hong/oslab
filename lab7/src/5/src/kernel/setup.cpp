...
void test_thread(void *arg){
    ...
    {
        int vaddr = memoryManager.allocatePages(AddressPoolType::KERNEL, pagesPerAlloc);
        if (vaddr == 0)
        {
        ...
        // 释放
        memoryManager.releasePages(AddressPoolType::KERNEL, vaddr, pagesPerAlloc);
        ++successCount;
    }

    printf("Test finished: total success = %d allocations of %d pages\n", successCount, pagesPerAlloc);
    asm_halt();
}
...
extern "C" void setup_kernel()
{
    ...
    // 创建第一个线程
    int pid = programManager.executeThread(test_thread, nullptr, "test thread", 1);
    if (pid == -1)
    {
